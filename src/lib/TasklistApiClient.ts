...
import {
  ...
  decodeTaskVariablesFromGraphQL,
...
export class TasklistApiClient {
   ...
   *
   */
  constructor(private authApiClient: TasklistAuthProvider) {
    this.userAgentString = `tasklist-graphql-client-nodejs/${pkg.version}`;
    const baseUrl = this.authApiClient.getBaseUrl();
    this.graphqlUrl = `${baseUrl}/graphql`;
  }

  private async getHeaders() {
    const authHeaders = await this.authApiClient.getHeaders({
  ...
  public async getTasks<T = { [key: string]: any }>(
    ...
    return gotQl.query(this.graphqlUrl, q, { headers }).then((res) => ({
      tasks: getResponseDataOrThrow<{ tasks: Task[] }>(res).tasks.map(
        decodeTaskVariablesFromGraphQL<T>
      ),
    }));
  }

  public async getAllTasks<T = { [key: string]: any }>(
    fields: TaskFields = defaultFields
  ): Promise<{ tasks: TaskWithVariables<T>[] }> {
   ...
   * @returns
   */
  public async getTask<T = { [key: string]: any }>(
    id: string,
    fields = defaultFields
  ): Promise<{ task: TaskWithVariables<T> }> {
    const headers = await this.getHeaders();
    const query = {
      operation: {
        name: "task",
    ...
    return gotQl.query(this.graphqlUrl, query, { headers }).then((res) => ({
      task: decodeTaskVariablesFromGraphQL<T>(
        getResponseDataOrThrow<{ task: Task }>(res).task
      ),
    }));
  }
  ...
  public async claimTask(
    taskId: string,
    assignee: string,
    allowOverrideAssignment: boolean = true
  ): Promise<{ claimTask: Task }> {
    const headers = await this.getHeaders();
    const query = {
      operation: {
        name: "claimTask",
  ...
  public async completeTask(
    ...
    return gotQl
      .mutation(this.graphqlUrl, query as any, { headers })
      .then((res) => getResponseDataOrThrow(res));
  }
  ...
  public async deleteProcessInstance(
  ...
  ): Promise<{ deleteProcessInstance: boolean }> {
    const headers = await this.getHeaders();
    const query = {
      operation: {
    ...
    return gotQl
      .mutation(this.graphqlUrl, query, { headers })
      .then((res) => getResponseDataOrThrow(res));
  }
