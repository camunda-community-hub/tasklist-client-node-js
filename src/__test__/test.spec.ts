...
import { SaasAuthProvider } from "../lib/auth-providers/SaasAuthProvider";
...
describe("TasklistApiClient", () => {
  ...
  it("can request all tasks", async () => {
    const tasklist = new TasklistApiClient(new SaasAuthProvider());
  ...
  it("can delete a process instance", async () => {
    const tasklist = new TasklistApiClient(new SaasAuthProvider());
    const { tasks } = await tasklist.getTasks({ state: TaskState.COMPLETED }, [
      "processInstanceId",
    ]);
    expect(tasks.length).toBeGreaterThan(0);
